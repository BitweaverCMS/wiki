{strip}
{if $gBitSystem->isPackageActive( 'wiki' ) and $modLastModif}
	{bitmodule title="$moduleTitle" name="last_modif_pages"}
		...
		{if $gBitUser->hasPermission( 'p_wiki_list_pages') }
			<a class="more" href="{$smarty.const.WIKI_PKG_URL}list_pages.php?sort_mode=last_modified_desc">Show More&hellip;</a>
		{/if}
	{/bitmodule}
{/if}
{/strip}

...
<div class="display wiki">
	<div class="floaticon"> <!-- Actions -->
		...
		{/if}
		{if $gBitSystem->isPackageActive( 'pdf' ) and $gContent->hasUserPermission( 'p_pdf_generation' )}
			<a title="create PDF" href="{$smarty.const.PDF_PKG_URL}?page_id={$gContent->mInfo.page_id}">{biticon ipackage="pdf" iname="pdf" iexplain="PDF"}</a>
		{/if}
	</div> <!-- End Actions -->

...
<div class="display wiki">
	<nav class="floaticon"> <!-- Actions -->
		...
		{/if}
		{if $gBitSystem->isPackageActive( 'pdf' ) and $wikiPage->hasUserPermission( 'p_pdf_generation' )}
			<a title="create PDF" href="{$smarty.const.PDF_PKG_URL}?page_id={$wikiPage->mInfo.page_id}">{biticon ipackage="pdf" iname="pdf" iexplain="PDF"}</a>
		{/if}
	</nav> <!-- End Actions -->

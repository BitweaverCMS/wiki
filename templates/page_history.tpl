...
<div class="admin wiki">
	<div class="header">
		<h1>History of <a href="{$gContent->mInfo.display_url}">{$gContent->mInfo.title|escape}</a></h1>
	</div>
	...
	<div class="body">
		...
		{if $compare eq 'y'}
			<table class="table data">
				<caption>Comparing versions</caption>
				<tr>
					<th width="50%">Version {$version_from}</td>
					<th></th>
					<th width="50%">Current version</td>
				</tr>
				<tr class="aligntop">
		...
		{if $diff2 eq 'y'}
			<h2>Differences from version {$version_from} to {$version_to}</h2>
		...
		{form}
			...
			<table class="table data">
				...
				<tr>
					<th style="width:70%;">Date/Comment</th>
				...
				<tr class="odd">
					<td colspan="4">
						<a href="{$gContent->mInfo.display_url}">Current</a>
			...
			{if $gBitUser->hasPermission( 'p_wiki_remove_page' )}
				<div style="text-align:right;">
					<input type="submit" class="btn btn-default" name="delete" value="Delete selected versions" />
				</div>
			{/if}
		{/form}

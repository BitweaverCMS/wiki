...
<div class="listing wiki">
	<header>
		...
		{form class="form-inline" }
			...
			<label>Title: <input class="input-small" type="text" name="find_title" value="{$find_title|default:$smarty.request.find_title|escape}" /></label>  <label>Author: <input class="input-small" type="text" name="find_author" value="{$find_author|default:$smarty.request.find_author|escape}" /></label> <label>Last Editor: <input class="input-small" type="text" name="find_last_editor" value="{$find_last_editor|default:$smarty.request.find_last_editor|escape}" /></label>  <input type="submit" class="btn btn-xs" name="search" value="Find" /> 
		{/form}
		</div>
		<h1>{$gBitSystem->getBrowserTitle()}</h1>
	</header>
	...
	<div class="body">
		...
		{form id="checkform"}
			...
			<table class="table data">
				<caption>WikiPages Listing <span class="total">[ {$listInfo.total_records} ]</span></caption>
				<tr>
				...
				{section name=changes loop=$listpages}
					<tr class="{cycle advance=false}">
						<td colspan="{$cols}">
							...
							{if $gBitSystem->isFeatureActive( 'wiki_list_creator' ) and $gBitSystem->isFeatureActive( 'wiki_list_lastmodif' ) }
								<span style="display:block; width:50%; float:left;">
									Created: {displayname real_name=$listpages[changes].creator_real_name user=$listpages[changes].creator_user}
								...
								<span style="display:block; width:50%; float:right;">
									...
									{else}
										Last Edited: {displayname real_name=$listpages[changes].modifier_real_name user=$listpages[changes].modifier_user}
										, {$listpages[changes].last_modified|bit_short_datetime}
									{/if}
								</span>
							{else}
								{if $gBitSystem->isFeatureActive( 'wiki_list_creator' )}
									Created by {displayname real_name=$listpages[changes].creator_real_name user=$listpages[changes].creator_user}
								{/if}
								, {$listpages[changes].created|bit_short_datetime}
								{if $gBitSystem->isFeatureActive( 'wiki_list_lastmodif' ) and ($listpages[changes].version > 1)}
									<br />
									Last modified
									{if $listpages[changes].editor != $listpages[changes].creator}
										&nbsp;by {displayname real_name=$listpages[changes].modifier_real_name user=$listpages[changes].modifier_user}
									{/if}
									, {$listpages[changes].last_modified|bit_short_datetime}
								{/if}
							{/if}
					...
					<tr class="{cycle}">
						...
						{/if}
						{if $gBitSystem->isFeatureActive( 'wiki_list_backlinks' )}
							{if $gBitSystem->isFeatureActive( 'wiki_backlinks' ) and $listpages[changes].backlinks > 0}
			...
			{if $checkboxes_on eq 'y'}
				<div style="text-align:right;">
					<script>/* <![CDATA[ check / uncheck all */
						document.write("<label for=\"switcher\">Select All <input name=\"switcher\" id=\"switcher\" type=\"checkbox\" onclick=\"BitBase.switchCheckboxes(this.form.id,'checked[]','switcher')\" /></label>");
					/* ]]> */</script>
					<br />

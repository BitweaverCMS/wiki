<div class="display copyrights">
	<div class="header">
		<h1>Copyrights for {$gContent->mInfo.title|escape}</h1>
	</div>
	...
	{legend legend="Copyright settings"}
		{section name=i loop=$copyrights}
			{form}
				...
				<div class="form-group submit">
					...
					<input type="submit" class="btn btn-default" name="editcopyright" value="edit" />
					<a href="{$smarty.const.WIKI_PKG_URL}copyrights.php?page_id={$gContent->mInfo.page_id}&amp;action=up&amp;copyright_id={$copyrights[i].copyright_id}">{booticon iname="icon-cloud-upload" ipackage="icons" iexplain="move up"}</a>
					<a href="{$smarty.const.WIKI_PKG_URL}copyrights.php?page_id={$gContent->mInfo.page_id}&amp;action=down&amp;copyright_id={$copyrights[i].copyright_id}">{booticon iname="icon-cloud-download"  ipackage="icons"  iexplain="move down"}</a>
					<a title="Delete this copyright" href="{$smarty.const.WIKI_PKG_URL}copyrights.php?page_id={$gContent->mInfo.page_id}&amp;action=delete&amp;copyright_id={$copyrights[i].copyright_id}" 
						onclick="return confirm('Are you sure you want to delete this copyright?')">{booticon iname="icon-trash" ipackage="icons" iexplain="delete"}</a>
				</div>
			{/form}
		{/section}
		{/legend}
		...
		{form legend="Add a new copyright setting"}
			...
			<div class="form-group submit">
				<input type="submit" class="btn btn-default" name="addcopyright" value="add" />
			</div>
		{/form}
	</div><!-- end .body -->
</div>

...
{if $gBitUser->hasPermission( 'p_users_view_icons_and_tools')}
	<div class="floaticon">
		...
		{if !empty($structureInfo.structure_id) and ($gStructure->mInfo.creator_user_id == $gBitUser->mUserId or $gContent->hasUserPermission( 'p_wiki_admin_book' ))}
		...
		{elseif $gContent->mType =='BitBook' and $gContent->hasUpdatePermission()}
		...
		{if $gBitUser->isRegistered() and $gBitUser->mUserId and $gBitSystem->isFeatureActive( 'users_watches' ) }
			{if $user_watching_page eq 'y'}
				<a title="stop monitoring this page" href="{$smarty.const.WIKI_PKG_URL}index.php?page_id={$gContent->mInfo.page_id}&amp;watch_event=wiki_page_changed&amp;watch_object={$gContent->mInfo.page_id}&amp;watch_action=remove">{booticon ipackage="icons" iname="icon-eye-close" iexplain="Stop monitoring"}</a>
			{else}
				<a title="monitor this page" href="{$smarty.const.WIKI_PKG_URL}index.php?page_id={$gContent->mInfo.page_id}&amp;watch_event=wiki_page_changed&amp;watch_object={$gContent->mInfo.page_id}&amp;watch_action=add">{booticon iname="icon-eye"  ipackage="icons"  iexplain="Monitor"}</a>
			{/if}
		{/if}
...
*}
		...
		{if $gContent->mInfo.title ne 'SandBox' and !$gContent->isLocked()}
			{if $gContent->isOwner() or $gContent->hasUserPermission( 'p_wiki_remove_page' )}
				<a title="Remove this page" href="{$smarty.const.WIKI_PKG_URL}remove_page.php?page_id={$gContent->mInfo.page_id}&amp;version=last">{booticon iname="icon-trash" ipackage="icons" iexplain="Delete"}</a>
			{/if}
		{/if}
		...
		{if !empty($showstructs) and count($showstructs) gt 0}
			<select id="sel-structures" name="page" onchange="javascript:BitBase.go(this)">
				<option value="">Wiki Books &hellip;</option>
